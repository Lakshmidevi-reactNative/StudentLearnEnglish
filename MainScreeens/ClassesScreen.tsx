...
export default function ClassesScreen() {
  ...
  const handleAttemptAssignment = (item) => {
    ...
    if (item.type === "RolePlay Practice") {
      ...
      // Fallback for other types - convert to one of the three required types
      const defaultType = "Language Practice";
      toast.success(`Starting ${defaultType} assignment: ${item.title}`);
      navigation.navigate("LanguageAttempt", { assignment: {...item, type: defaultType} });
    }
  };
  
  const handleViewResults = (item) => {
    // Navigate to the appropriate results screen based on type
    if (item.type === "RolePlay Practice") {
      navigation.navigate("RoleplayResult", { 
        studentId: 977,
        assignmentId: 3085,
        classId: 12294,
        assignment: item 
      ...
      navigation.navigate("TypingResult", { 
        studentId: 948,
        assignmentId: 3063,
        classId: 12099,
        assignment: item 
      ...
      navigation.navigate("LanguageResult", { 
        ...
        assignmentId: 3085,
        classId: 12294,
        assignment: {...item, type: "Language Practice"} 
      });
    }
  };
	...
	const getContentTypeIcon = (type) => {
		// Normalize the type to one of the three required types
	...
	// Helper function to normalize content types to one of the three required types
	const normalizeContentType = (type) => {
		if (!type) return "Language Practice";
		
		const typeStr = type.toLowerCase();
		if (typeStr.includes("language") || typeStr.includes("multiple choice") || 
			  typeStr.includes("fill") || typeStr.includes("matching") || 
			  typeStr.includes("oral") || typeStr.includes("audio")) {
			return "Language Practice";
		} else if (typeStr.includes("typing") || typeStr.includes("writing") || 
			       typeStr.includes("essay") || typeStr.includes("text")) {
			return "Typing Practice";
		} else if (typeStr.includes("role") || typeStr.includes("play") || 
			       typeStr.includes("dialogue") || typeStr.includes("interaction")) {
	...
	const renderAssignmentItem = ({ item }) => {
		...
		return (
			<TouchableOpacity
				...
				onPress={() => handleContentItemPress(normalizedItem)}
			>
				<View style={styles.contentItemHeader}>
					...
					>
						<MaterialCommunityIcons
							name={getContentTypeIcon(normalizedItem.type)}
							size={20}
							color={COLORS.neonBlue}
						/>
					</View>
					<View style={styles.contentItemTitleContainer}>
						<Text style={styles.contentItemTitle}>{normalizedItem.title}</Text>
						<View style={styles.contentItemType}>
							<Text style={styles.contentItemTypeText}>{normalizedItem.type}</Text>
						</View>
					</View>
					<View
						style={[
							styles.statusBadge,
							{ backgroundColor: `${getStatusColor(normalizedItem.status)}20` },
						]}
					>
						<Text
							style={[styles.statusText, { color: getStatusColor(normalizedItem.status) }]}
						>
				...
				<View style={styles.contentItemInfo}>
					<View style={styles.contentItemInfoRow}>
						...
						<Text style={styles.contentItemInfoText}>
							Due:{" "}
							{new Date(normalizedItem.dueDate).toLocaleDateString("en-US", {
								month: "short",
								day: "numeric",
								year: "numeric",
							})}
					...
					<View style={styles.contentItemInfoRow}>
						<MaterialCommunityIcons
							...
							color={COLORS.textSecondary}
						/>
						<Text style={styles.contentItemInfoText}>{normalizedItem.duration}</Text>
					</View>
					<View style={styles.contentItemInfoRow}>
						<MaterialCommunityIcons
						...
						/>
						<Text style={styles.contentItemInfoText}>
							{normalizedItem.questions} question{normalizedItem.questions !== 1 ? "s" : ""}
						</Text>
					</View>
				</View>
				...
				{normalizedItem.preview && (
					<View style={styles.previewContainer}>
						...
						<Text style={styles.previewText}>{normalizedItem.preview}</Text>
					</View>
				)}

				<View style={styles.contentItemFooter}>
					{normalizedItem.status?.toLowerCase() === "pending" ? (
						<TouchableOpacity
							style={styles.attemptButton}
							onPress={() => handleAttemptAssignment(normalizedItem)}
						>
						...
						<TouchableOpacity 
							...
							onPress={() => handleViewResults(normalizedItem)}
						>
							<Text style={styles.reviewButtonText}>View Results</Text>
						</TouchableOpacity>
					)}
				</View>
			</TouchableOpacity>
				...
				<View style={styles.assessmentDetailsContainer}>
					<View style={styles.assessmentDetailsRow}>
						...
						<View style={styles.assessmentDetailItem}>
							...
							<Text style={styles.assessmentDetailValue}>
								{new Date(normalizedItem.date).toLocaleDateString("en-US", {
									month: "short",
									day: "numeric",
								})}
							</Text>
	...
	const renderDetailPage = () => {
		...
		const normalizedItem = {
			...selectedContentItem,
			type: normalizeContentType(selectedContentItem?.type)
		};
			...
			<>
				...
				>
					...
					<View style={styles.detailContentContainer}>
						...
						<Text style={styles.detailTitle}>{normalizedItem?.title}</Text>
						...
						<View style={styles.detailMetaContainer}>
							...
							<View style={styles.detailMetaItem}>
								...
								/>
								<Text style={styles.detailMetaText}>
									{normalizedItem?.questions} question
									{normalizedItem?.questions !== 1 ? "s" : ""}
								</Text>
							</View>
							<View style={styles.detailMetaDot} />
							<View style={styles.detailMetaItem}>
						...
						<View
							style={[
								styles.detailStatusBadge,
								{
									backgroundColor:
										selectedContentType === "assignments"
											? `${getStatusColor(normalizedItem?.status)}20`
											: `${COLORS.neonGreen}20`,
								},
							]}
						>
							<Text
								style={[
									styles.detailStatusText,
									{
										color:
											selectedContentType === "assignments"
												? getStatusColor(normalizedItem?.status)
												: COLORS.neonGreen,
									},
								]}
							>
